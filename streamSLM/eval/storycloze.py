#!/usr/bin/env python3
"""StoryCloze evaluation for StreamSLM.

Mirrors TASTE-SpokenLM/storycloze.py: each leaf folder (sSC = spoken,
tSC = textual-style synthetic) holds files named ``<idx>_<uuid>_<correct
|incorrect>.wav``. Pairs are formed by adjacent indices — even idx is
the positive (correct) continuation, odd idx is the negative.

For each pair we compute a streamSLM score on (audio, streaming-ASR
TextGrid) and count the pair correct iff ``score_pos > score_neg``. The
TextGrid is the chunk-level transcription produced by the streaming
Conformer-Transducer (one ``<wav_stem>.TextGrid`` per wav under
``--tg_root``), the same alignment signal used to build the SLM
training data. Whisper transcripts are no longer consulted because
they leak future context into the constrained-Viterbi alignment.

The scoring formula is selected by ``--scoring_mode``:

* ``likelihood``: joint log-likelihood, sum reduction.
* ``loss`` (default): TASTE-style sum of per-stream mean cross-entropy
  values (text mean + sum of R per-channel acoustic means).

Usage (1× 48 GB GPU, low quota):

    sr 1 48 --qos=q-low python -m streamSLM.eval.storycloze \\
        --slm_checkpoint checkpoints/streamSLM/.../step_00050000.pt \\
        --datasets       sSC \\
        --output_dir     results/storycloze

TextGrids default to alongside the wavs under ``--data_root``; pass
``--tg_root`` only if they live in a separate mirror.

Results are nested under ``<output_dir>/<ckpt_tag>/<mode>/``.
"""
...
# Allow ``python -m streamSLM.eval.storycloze`` from the repo root.
...
def _load_pairs(folder: Path, tg_folder: Path) -> List[dict]:
    """Build (pos, neg) pairs from filenames.

    Even idx = positive (``*_correct.wav``), odd idx = negative
    (``*_incorrect.wav``). The two share the same UUID stem and each must
    have a mirrored ``<stem>.TextGrid`` under ``tg_folder``. Wavs whose
    TextGrid is missing are dropped.
    """
    by_idx: dict = {}
    missing_tg = 0
    ...
    for idx in sorted(by_idx.keys()):
        if idx % 2 != 0:
            continue
        if idx + 1 not in by_idx:
            continue
        pairs.append({"ind": idx,
                      "positive": by_idx[idx],
                      "negative": by_idx[idx + 1]})
    if missing_tg:
        print(f"[load_pairs] {folder.name}: {missing_tg} wavs dropped — "
              f"TextGrid missing under {tg_folder}", flush=True)
    return pairs
...
def _score_one(extractor, scorer, wav_path: str, tg_path: str) -> dict:
...
def evaluate(extractor, scorer, pairs: List[dict], dataset_name: str) -> dict:
    correct = 0
    total = 0
    invalid = 0
    scores = []
    pbar = tqdm(pairs, desc=dataset_name, ncols=100)
    for pair in pbar:
        try:
            ...
            print(f"[{dataset_name}] pair {pair['ind']} failed: {e}", flush=True)
            invalid += 1
            continue
    ...
    return {
        ...
        "text_weight":         scorer.text_weight,
        "acoustic_weight":     scorer.acoustic_weight,
        "acoustic_ch_weights": scorer.acoustic_ch_weights,
        "accuracy":            accuracy,
        "correct":             correct,
        "total":               total,
        "invalid":             invalid,
...
def main():
    ...
    ap.add_argument("--data_root", default=os.environ.get("STORYCLOZE_DATA_ROOT"),
                    required="STORYCLOZE_DATA_ROOT" not in os.environ,
                    help="StoryCloze dataset root (env: STORYCLOZE_DATA_ROOT).")
    ap.add_argument("--tg_root", default=None,
                    help="Directory holding <subset>/<stem>.TextGrid files "
                         "generated by streamSLM/scripts/run_eval_tg_gen.sh. "
                         "Defaults to --data_root (TGs are written alongside "
                         "the wavs).")
    ap.add_argument("--datasets", nargs="+", default=["sSC", "tSC"],
                    help="Subset(s) of StoryCloze to evaluate (default: sSC and tSC).")
    ap.add_argument("--output_dir", default="results/storycloze")
    ...
    ap.add_argument("--scoring_mode", choices=list(SCORING_MODES),
                    ...
                    help="Pair-classification score: 'loss' (default) = "
                         ...
                         "log-likelihood (sum reduction).")
    ap.add_argument("--text_weight", type=float, default=1.0,
                    help="Weight on the text-stream term when combining the "
                         "text and acoustic losses into the pair score "
                         "(default 1.0). With --acoustic_weight this sets "
                         "the text:acoustic ratio.")
    ap.add_argument("--acoustic_weight", type=float, default=1.0,
                    help="Weight on the acoustic-stream term in the pair "
                         "score (default 1.0).")
    ap.add_argument("--acoustic_ch_weights", default="",
                    help="Comma-separated per-codebook weights (length R) "
                         "applied to the acoustic CEs in 'loss' mode before "
                         "summing, e.g. the training rvq_loss_weights "
                         "'10,3,3,3,3,3,3,3,1,1,1,1,1,1,1,1'. Empty (default) "
                         "= uniform sum over the R codebooks.")
    ...
    for ds in args.datasets:
        folder = Path(args.data_root) / ds
        tg_folder = Path(tg_root) / ds
        if not folder.exists():
            print(f"[skip] {ds}: {folder} missing", flush=True)
            continue
        if not tg_folder.exists():
            print(f"[skip] {ds}: TextGrid folder {tg_folder} missing — "
        ...
        if sharded:
            ...
            print(f"[ds  ] {ds}: shard {args.rank}/{args.world} got "
                  f"{len(pairs)} pairs from {folder}", flush=True)
        else:
            print(f"[ds  ] {ds}: {len(pairs)} pairs from {folder}", flush=True)
        if not pairs:
            continue

        t0 = time.time()
        result = evaluate(extractor, scorer, pairs, ds)
        elapsed = time.time() - t0
        print(f"[ds  ] {ds}: acc={result['accuracy']:.4f} "
              f"({result['correct']}/{result['total']}) "
              f"invalid={result['invalid']} in {elapsed:.0f}s", flush=True)

        out_path = out_base / f"storycloze_{ds}{suffix}.json"
    ...
    if sharded:
        summary_path = out_base / f"storycloze_summary{suffix}.json"
    else:
        summary_path = out_base / "storycloze_summary.json"
    with open(summary_path, "w") as f:
        json.dump(
            {"scoring_mode":        args.scoring_mode,
             ...
             "acoustic_weight":     args.acoustic_weight,
             "acoustic_ch_weights": scorer.acoustic_ch_weights,
             "datasets":            overall_summary},
            f, indent=2,
        )
    print(f"[done] scoring_mode={args.scoring_mode} summary -> {summary_path}",
          flush=True)
