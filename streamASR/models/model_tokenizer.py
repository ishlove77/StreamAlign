...
class Data2VecSemanticAcousticModel(nn.Module):
    def __init__(self, vocab_size: int = None, *,
        ...
        # Encoder (StreamingCharModel/Conformer) hidden state dim — fixed by
        # the encoder architecture and independent of the decoder's hidden dim.
        d_enc_hidden = 512
        # Decoder transformer hidden dim. Override via env D_HIDDEN to match a
        # foreign init checkpoint (e.g. SubAlign uses 768).
        d_hidden = int(os.environ.get("D_HIDDEN", "512"))
        self.num_layers = 12
        ...
        self.acoustic_head = nn.Sequential(
            nn.Linear(d_enc_hidden * self.concat_layers, d_hidden),
            nn.ReLU(),
            nn.Dropout(0.1),
        ...
        # Plain RVQ on feat_dim (no bottleneck). RVQ depth via env var RVQ_R,
        # per-layer codebook size via RVQ_CODEBOOK_SIZE. Optional low-dim
        # projection inside ResidualVQ via RVQ_CODEBOOK_DIM (omit to keep
        # codebook_dim == feat_dim, the legacy behavior).
        self.rvq_r = int(os.environ.get("RVQ_R", "3"))
        self.codebook_size = int(os.environ.get("RVQ_CODEBOOK_SIZE", "512"))
        # RVQ_BYPASS=1 -> continuous stage: skip quantization entirely
        # (see _rvq_quantize). The quantizer is still constructed so that the
        # resulting checkpoint can initialize the RVQ stage that follows.
        self.rvq_bypass = os.environ.get("RVQ_BYPASS", "0") == "1"
        rvq_kwargs = dict(
            dim=feat_dim,
            num_quantizers=self.rvq_r,
            codebook_size=self.codebook_size,
            kmeans_init=True,
            kmeans_iters=100,
            threshold_ema_dead_code=2,
            commitment_weight=float(os.environ.get("COMMIT_WEIGHT", "1.0")),
        )
        if os.environ.get("RVQ_CODEBOOK_DIM"):
            rvq_kwargs["codebook_dim"] = int(os.environ["RVQ_CODEBOOK_DIM"])
        self.residual_vq = ResidualVQ(**rvq_kwargs)

        # MSE distillation: MSE(z_q, z_cont.detach()) with this weight.
        self.distill_weight = float(os.environ.get("DISTILL_WEIGHT", "0"))
    ...
    def _build_alignment_from_rnnt(
        ...
        char_ids_list=None, constraints_list=None,
    ):
        """Convert RNN-T decode output into alignment matrices and char_data.

        When ``char_ids_list`` and ``constraints_list`` are provided, char_data
        is built directly from the per-interval ground-truth via
        ``char_data_from_constraints`` — each TextGrid interval is tokenized
        independently with a leading-space prefix so the LLM tokenizer emits
        proper word-leading subwords (e.g. ``" think"`` → 1781) instead of
        word-internal variants. The returned ``char_indices`` exactly match
        the input ``cids`` so they line up one-to-one with
        ``frame_positions[b][i]``.

        Without those args, falls back to the legacy path that decodes
        ``self.encoder.tokenizer`` on the RNN-T predictions and re-tokenizes
        with the LLM tokenizer — that path concatenates words without spaces
        (the char vocab strips inter-word spaces), which causes the LLM
        tokenizer to choose word-internal subword variants.
        """
        B = len(predictions)
        ...
        if use_constraints:
            from utils.text_utils import char_data_from_constraints
            char_batch_meta = char_data_from_constraints(
                tokenizer, char_ids_list, constraints_list
            )
        else:
    ...
    def forward(
        ...
        char_alignment, word_alignment, char_data, gt_char_indices = \
            self._build_alignment_from_rnnt(
                ...
                char_ids_list=char_ids_list if use_constrained else None,
                constraints_list=constraints_list if use_constrained else None,
            )
    ...
    def forward_streaming(
        ...
        for idx, chunk in enumerate(all_chunks):
            ...
            seg_z_q, _ = self._rvq_quantize(seg_z)
            seg_feat = torch.cat([
                seg_word_emb,
                seg_z_q,
    ...
    # ------------------------------------------------------------------ #
    #  Per-subword (w_n, q_n, d_n) extraction for downstream SLM training
    # ------------------------------------------------------------------ #
    @torch.no_grad()
    def extract_subword_units(
        self,
        waveforms: torch.Tensor,
        wav_mask: torch.Tensor,
        tokenizer,
        ...
        """Return per-subword units used by streamSLM training.

        Reuses the constrained-RNNT alignment + per-word feature pooling +
        residual VQ quantizer from the trained model, then surfaces the
        codebook indices and per-word frame counts that the standard forward
        pass discards.

        Returns
        -------
        list[dict] of length B; each entry has
            subword_ids     : LongTensor (N,)         LLM-tokenizer vocab ids
            q_codes         : LongTensor (N, R)       RVQ codebook indices
            duration_frames : LongTensor (N,)         encoder-frame count per subword
            pre_quant_feat  : FloatTensor (N, D)      seg_z slice fed into residual_vq;
                                                      D = self.residual_vq.dim (typically 256)
        where N is the number of subwords in that utterance. Durations are
        kept in frame units to avoid baking in the encoder hop; convert to
        seconds downstream if needed (compute_model_feat_len -> 40 ms today).
        """
        from utils.rnnt_align_loss import constrained_rnnt_align
        ...
        enc_out, hidden = self.encoder(
            waveforms, input_len_relative, self.chunk_config, epoch=epoch
        )
        top_half_layers = hidden[1 : (self.concat_layers + 1)]
        h_aco_raw = torch.cat(top_half_layers, dim=-1)
        hubert_feat_lens = torch.tensor(
        ...
        char_alignment, word_alignment, char_data, _ = self._build_alignment_from_rnnt(
            predictions, frame_positions, T_speech,
            tokenizer, txt_normalizer, hubert_feat_lens,
        ...
        B = waveforms.size(0)
        empty = lambda R: [{
            "subword_ids":     torch.zeros(0, dtype=torch.long),
            "q_codes":         torch.zeros(0, R, dtype=torch.long),
            "duration_frames": torch.zeros(0, dtype=torch.long),
        } for _ in range(B)]

        if not (char_alignment.any() and word_alignment.any()):
            R = getattr(self.residual_vq, "num_quantizers",
                        len(getattr(self.residual_vq, "layers", [])) or 1)
            D = int(getattr(self.residual_vq, "dim", 256))
            return [
                {**e, "pre_quant_feat": torch.zeros(0, D, dtype=torch.float32)}
                for e in empty(R)
            ]

        aggregated, segment_info = self._pool_to_word_segments_transformer(
            z_raw, word_alignment, char_alignment,
        )
        T_words = word_alignment.size(1)
        feat_dim = aggregated.size(1)
        device = aggregated.device

        seg_z = self._reconstruct_word_features_batch(
            aggregated, segment_info, B, T_words, feat_dim, device,
        )
        # ResidualVQ returns (quantized, indices, loss);
        # indices are (B, T_words, R).
        _, q_indices, _ = self.residual_vq(seg_z)
        R = int(q_indices.size(-1))
        D = int(seg_z.size(-1))

        word_frame_counts = word_alignment.sum(dim=2)  # (B, T_words), int

        out = []
        for b in range(B):
            c2w = char_data[b]["char_to_word_map"]
            n_words = (max(c2w) + 1) if c2w else 0
            sw_list = char_data[b]["word_token_ids"]
            n = min(n_words, len(sw_list), T_words)
            if n == 0:
                out.append({
                    "subword_ids":     torch.zeros(0, dtype=torch.long),
                    "q_codes":         torch.zeros(0, R, dtype=torch.long),
                    "duration_frames": torch.zeros(0, dtype=torch.long),
                    "pre_quant_feat":  torch.zeros(0, D, dtype=torch.float32),
                })
                continue
            out.append({
                "subword_ids":     torch.tensor(sw_list[:n], dtype=torch.long),
                "q_codes":         q_indices[b, :n].detach().to(torch.long).cpu(),
                "duration_frames": word_frame_counts[b, :n].detach().to(torch.long).cpu(),
                # Float (caller decides dtype: extract_tokens.py downcasts to bf16
                # for the cache; training-side losses cast back to fp32 if needed).
                "pre_quant_feat":   seg_z[b, :n].detach().to(torch.float32).cpu(),
            })
        return out

    # ------------------------------------------------------------------ #
    #  Plain RVQ helper (no bottleneck)
    # ------------------------------------------------------------------ #
    def _rvq_quantize(self, seg_z: torch.Tensor):
        """RVQ directly on feat_dim. Returns (seg_z_q, vq_loss).

        With RVQ_BYPASS=1 the quantizer is skipped entirely and the continuous
        segment embedding is passed through unchanged (zero VQ loss). This is
        the "continuous representation" stage of the tokenizer pipeline, which
        is trained first and then used to initialize the R=16 RVQ stage.
        """
        if self.rvq_bypass:
            return seg_z, seg_z.new_zeros(())
        z_q, _, vq_loss = self.residual_vq(seg_z)
        aux = vq_loss.sum()
        if self.training and self.distill_weight > 0:
            aux = aux + self.distill_weight * F.mse_loss(z_q, seg_z.detach())
        return z_q, aux
    ...
    def _segment_pass(
        ...
        seg_z_q, vq_loss = self._rvq_quantize(seg_z)
        B, T_words, _ = seg_z_q.shape
        seg_feat = torch.cat([
            seg_word_emb,
            seg_z_q,
            spk_emb.unsqueeze(1).expand(-1, T_words, -1),
