...
class InputNormalization(torch.nn.Module):
    ...
    def forward(self, x, lengths=None, epoch=None):
        ...
        if self.norm_type == "global":
            ...
            mean = self.glob_mean.to(x.device) if isinstance(self.glob_mean, torch.Tensor) else self.glob_mean
            std  = self.glob_std.to(x.device)  if isinstance(self.glob_std,  torch.Tensor) else self.glob_std
    ...
    def _update_global_stats(self, x, mask):
        ...
        # Move running stats to the same device as x (needed for multi-GPU)
